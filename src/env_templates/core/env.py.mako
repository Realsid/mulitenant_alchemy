...
def set_schema(connection: Connection, context: context, schema: str) -> None:
    """Set the schema for the connection."""
    if connection.dialect.name == "postgresql":
        # set search path on the connection, which ensures that
        # PostgreSQL will emit all CREATE / ALTER / DROP statements
        # in terms of this schema by default
        context.execute(text('CREATE SCHEMA IF NOT EXISTS "%s"' % schema))
        context.execute(text('set search_path to "%s"' % schema))
        ...
        # in terms of this schema by default
        context.execute(text("CREATE DATABASE IF NOT EXISTS %s" % schema))
        # Create the schema if it does not exist
        context.execute(text("USE %s" % schema))
...
def order_columns(
    ...
    # pyright: ignore[reportUnknownVariableType,reportUnknownArgumentType,reportUnknownLambdaType]
...
def do_run_migrations(connection: Connection) -> None:
    ...
    with context.begin_transaction():
        set_schema(connection, context, SCHEMA)
        context.run_migrations()
